import HaditsDetailPage from "./_page-client";
...
export async function generateStaticParams() {
  try {
    ...
    if (!res.ok) return FALLBACK_SLUGS.map((slug) => ({ slug, nomor: "1" }));
    ...
    if (!Array.isArray(kitab) || kitab.length === 0) {
      return FALLBACK_SLUGS.map((slug) => ({ slug, nomor: "1" }));
    }
    return kitab.map((k) => ({ slug: k.slug, nomor: "1" }));
  } catch {
    return FALLBACK_SLUGS.map((slug) => ({ slug, nomor: "1" }));
  }
}
